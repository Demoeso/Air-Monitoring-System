...
void storeSampleInEeprom(int16_t tempTenths, int16_t humTenths) {
  ...
  // Because 'head' is oldest, the newest is at head+count-1
  int writeIndex = (eepromHead + eepromCount - 1) % MAX_SAMPLES;

  // Calculate byte address in EEPROM
  int sampleAddr = ADDR_DATA + writeIndex * 4;  // 4 bytes per record
...
// So the average & variance in RAM can be computed.
...
void setup() {
  ...
  // 2) Load all samples from EEPROM into temperatureData/humidityData arrays
...
void loop() {
  ...
  // 2) Store them in EEPROM ring buffer (scaled to tenths)
  int16_t tempTenths = (int16_t)(temperatureC * 10.0 + 0.5); // round to nearest tenth
  int16_t humTenths  = (int16_t)(humidity     * 10.0 + 0.5);
  storeSampleInEeprom(tempTenths, humTenths);

  // 3) Reload all samples into RAM (To compute avg/var)
  loadAllSamplesFromEeprom();

  // 4) Compute average & variance
  ...
  // Line 2: T avg & variance
  lcd.setCursor(0, 2);
  lcd.print("T Avg=");
  lcd.print(displayAvgTemp, 1);
  lcd.print(" Var=");
  lcd.print(varTempC, 0);

  // Line 3: H avg & variance
  lcd.setCursor(0, 3);
  lcd.print("H Avg=");
  lcd.print(avgHum, 0);
  lcd.print("% Var=");
  ...
  Serial.print("; VarT=");  Serial.print(varTempC);
  Serial.print("; Havg=");  Serial.print(avgHum);
  Serial.print("; VarH=");  Serial.println(varHum);

  // 6) Wait for next reading
  // For real usage: 1 hour => delay(3600000)
  // For Proteus demo, shorter:
  delay(3000);
}
